...
rm.writeClipImg = function (imgsrc) {
  ...
  console.log('按下复制');
  rm.hideRightMenu();
  mar.snackbarShow('正在下载中，请稍后', false, lazyTime)
  if (rm.downloadimging == false) {
    rm.downloadimging = true;
    setTimeout(function () {
      copyImage(imgsrc);
      mar.snackbarShow('复制成功！图片已添加水印，请遵守版权协议');
...
rm.copyPageUrl = function () {
  ...
  rm.copyUrl(url);
  mar.snackbarShow('复制本页链接地址成功', false, 2000);
  rm.hideRightMenu();
}
...
rm.searchBaidu = function () {
  mar.snackbarShow('即将跳转到百度搜索', false, 2000);
  setTimeout(function () {
...
function addRightMenuClickEvent() {
  ...
  $('#menu-top').on('click', function () { mar.scrollToDest(0, 500); rm.hideRightMenu(); });
  ...
  $('#menu-commentBarrage').on('click', mar.switchCommentBarrage);
  ...
  $('#menu-copytext').on('click', function () { rm.rightmenuCopyText(selectTextNow); mar.snackbarShow('复制成功，复制和转载请标注本文地址'); });
  ...
  $('#menu-downloadimg').on('click', function () { mar.downloadImage(domImgSrc, 'Marcus'); });
  $('#menu-copyimg').on('click', function () { rm.writeClipImg(domImgSrc); });
  $('#menu-searchBaidu').on('click', rm.searchBaidu);
  //音乐
  $('#menu-music-toggle').on('click', mar.musicToggle);
  $('#menu-music-back').on('click', mar.musicSkipBack);
  $('#menu-music-forward').on('click', mar.musicSkipForward);
  $('#menu-music-copyMusicName').on('click', function () { rm.rightmenuCopyText(mar.musicGetName()); mar.snackbarShow('复制歌曲名称成功', false, 3000); });
}