window.addEventListener('load', () => {
  ...
  const openSearch = () => {
    ...
    mar.animateIn($searchMask, 'to_show 0.5s')
    mar.animateIn(document.querySelector('#local-search .search-dialog'), 'titleScale 0.5s')
  ...
  const closeSearch = () => {
    ...
    bodyStyle.width = ''
    bodyStyle.overflow = ''
    mar.animateOut(document.querySelector('#local-search .search-dialog'), 'search_close .5s')
    mar.animateOut($searchMask, 'to_hide 0.5s')
  }
  ...
  window.addEventListener('pjax:complete', () => {
    !mar.isHidden($searchMask) && closeSearch()
    searchClickFn()
  })
})
