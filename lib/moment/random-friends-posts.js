var fdata = {
  apiurl: "https://fcircle.marcus233.top/",
  defaultFish: 100,
  hungryFish: 100,
}
...
function fetchRandomPost() {
  // 检查页面是否存在指定的元素
  var randomPostElement = document.getElementById("random-post");
  if (!randomPostElement) {
    return;
  }
  
  if (randomPostWorking) { // 如果fetchRandomPost正在运行，就直接返回
    return;
  } else {
